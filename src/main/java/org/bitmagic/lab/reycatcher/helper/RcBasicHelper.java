...
public class RcBasicHelper {
    ...
    public static void check(String usernameAndPwd, String realm) {
        ReqTokenInfo reqTokenInfo = TokenParseUtils.parseReqTokenInfo(RcRequestContextHolder.getContext().getRequest().getHeader("Authorization")).orElseThrow(() -> new BasicException("not found Authorization", realm));
